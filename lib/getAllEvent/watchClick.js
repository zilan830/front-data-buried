...
exports.WatchClick = function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (JSON.stringify(e) !== '{}') {
            ...
            var hasBusiness = pathArray_1.length > 0 && pathArray_1.filter(function (item) { return item.includes('btn') || item.includes('menu'); }).length > 0;
            var contentObj = {
                ...
                element: {
                    ...
                    // eslint-disable-next-line @typescript-eslint/camelcase
                    h_b: pathArray_1.length > 0 && pathArray_1.filter(function (item) { return item.includes('button') || item.includes('menu') || item.includes('tab') || item.includes('btn'); }).length > 0
                }
            };
